 ...
 * @author Piyush
 */
public class StateView extends PanelView implements InstantHelpListener, ConfigurationConstants {
	 ...
	 */
	public StateView(StateController controller, StateModel model) {
	...
	protected void instanciate() {
		...
		searchedCountry = new ValueObjectListTable(getCountryModel().getValueObjectModel(
				StateModel.SELECTED_COUNTRY_MODEL), getCountryModel().getValueObjectListModel(
				StateModel.SELECTED_COUNTRY_MODEL), new StateTableRowModel());
	...
	protected void compose() {
		...
		countrySearchPanel.addComponent(COUNTRY_NAME, countryNameTf, null, '\0', COUNTRY_CODE, countryCodeTf, null,
				'\0', false);
		countrySearchPanel.addButtonAction(getCountryController().getAction(StateController.ACTION_COUNTRY_SEARCH),
				false);
	...
	public StateController getCountryController() {
		return (StateController) getController();
	}
	
	public StateModel getCountryModel() {
		return (StateModel) getModel();
	}
