...
{
	int m_hgap; // horizontal gap

	int m_vgap; // vertical gap

	Dimension m_gridSize; // grid size in logical units (n x m)

	Hashtable m_compTable; // constraints (Rectangles)
	...
	public GraphPaperLayout(Dimension gridSize, int hgap, int vgap)
	{
		...
		this.m_gridSize = new Dimension(gridSize);
		this.m_hgap = hgap;
		this.m_vgap = vgap;
		m_compTable = new Hashtable();
	}
	...
	{
		return new Dimension(m_gridSize);
	}
	...
	{
		m_gridSize = new Dimension(width, height);
	}

	public void setConstraints(Component comp, Rectangle constraints)
	{
		m_compTable.put(comp, new Rectangle(constraints));
	}
	...
	protected Dimension getLayoutSize(Container parent, boolean isPreferred)
	{
		...
		largestSize.width = (largestSize.width * m_gridSize.width) + (m_hgap * (m_gridSize.width + 1)) + insets.left
		        + insets.right;
		largestSize.height = (largestSize.height * m_gridSize.height) + (m_vgap * (m_gridSize.height + 1)) + insets.top
		        + insets.bottom;
		return largestSize;
	}
	...
	protected Dimension getLargestCellSize(Container parent, boolean isPreferred)
	{
		...
		for (int i = 0; i < ncomponents; i++)
		{
			...
			Rectangle rect = (Rectangle) m_compTable.get(c);
			if (c != null && rect != null)
			{
	...
	public void layoutContainer(Container parent)
	{
		synchronized (parent.getTreeLock())
		{
			...
			int totalCellW = totalW / m_gridSize.width;
			int totalCellH = totalH / m_gridSize.height;

			// Cell dimensions, without padding
			int cellW = (totalW - ((m_gridSize.width + 1) * m_hgap)) / m_gridSize.width;
			int cellH = (totalH - ((m_gridSize.height + 1) * m_vgap)) / m_gridSize.height;
			...
			for (int i = 0; i < ncomponents; i++)
			{
				...
				if (rect != null)
				{
					int x = insets.left + (totalCellW * rect.x) + m_hgap;
					int y = insets.top + (totalCellH * rect.y) + m_vgap;
					int w = (cellW * rect.width) - m_hgap;
					int h = (cellH * rect.height) - m_vgap;
					c.setBounds(x, y, w, h);
				}
			}
		}
