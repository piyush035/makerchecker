...
public final class FormAttachment
{
	 ...
	 * a control. For top and bottom attachments, TOP, BOTTOM and CENTER are
	 * used. For left and right attachments, LEFT, RIGHT and CENTER are used. If
	 * any other case occurs, the default will be used instead.
	 * 
	 * Possible values are:
	 * 
	 * TOP: Attach the side to the top side of the specified control. BOTTOM :
	 * Attach the side to the bottom side of the specified control. LEFT: Attach
	 * the side to the left side of the specified control. RIGHT: Attach the
	 * side to the right side of the specified control. CENTER: Attach the side
	 * at a position which will center the control on the specified control.
	 ...
	 * Since no denominator is specified, the default is to read the numerator
	 ...
	 */
	public FormAttachment(Component control, int offset, int alignment)
	{
	 ...
	 *            the offset of the side from the control
	 */
	public FormAttachment(Component control, int offset)
	{
		this(control, offset, SWTConstants.DEFAULT);
	}
	 ...
	 *            the control the side is attached to
	 */
	public FormAttachment(Component control)
	{
		this(control, 0, SWTConstants.DEFAULT);
	}

	FormAttachment divide(int value)
	{
		return new FormAttachment(m_numerator, m_denominator * value, m_offset / value);
	}
	...
	FormAttachment minus(FormAttachment attachment)
	{
		...
		solution.m_numerator = m_numerator * attachment.m_denominator - m_denominator * attachment.m_numerator;
		solution.m_denominator = m_denominator * attachment.m_denominator;
		int gcd = gcd(solution.m_denominator, solution.m_numerator);
		solution.m_numerator = solution.m_numerator / gcd;
		solution.m_denominator = solution.m_denominator / gcd;
		solution.m_offset = m_offset - attachment.m_offset;
		return solution;
	}

	FormAttachment minus(int value)
	{
		return new FormAttachment(m_numerator, m_denominator, m_offset - value);
	}
	...
	int solveX(int value)
	{
		...
		return ((m_numerator * value) / m_denominator) + m_offset;
	}
	...
	int solveY(int value)
	{
		if (m_numerator == 0)
		{
			throw new IllegalArgumentException("numerator can not be zero.");
		}

		return (value - m_offset) * m_denominator / m_numerator;
	}

	public String toString()
	{
		String string = m_control != null ? m_control.toString() : m_numerator + "/" + m_denominator;
		return "y = (" + string + (m_offset >= 0 ? ")x + " + m_offset : ")x - " + (-m_offset));
	}
}